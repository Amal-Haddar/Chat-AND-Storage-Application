  ...
  async ChangePassword(req, res){
   ...
   async DeleteUser(req, res){
    ...
    try{
      ...
      // const result2 = await Post.find({"username" : { "$in": ["amal"] },});
      // console.log(result2);
      
      if(!result){
        return res.status(httpStatus.NOT_FOUND).json({ message:"Could not delete user"})
      } else {

        for (i = 0; i < result.posts.length ; i++){
          await Post.findByIdAndRemove(result.posts[i].postId)
        }

        for (i = 0; i < result.groups.length ; i++){

          await Group.update({ 
             _id:result.groups[i].groupId
          }, { 
               $pull: {members: {
                      userId: result._id
               }}
      ...
      //  for (i = 0; i < result2.comments.length; i++) {
      //    if (result2.comments[i].username === result.username){
      //      await Post.update({
      //       _id:result2.comments[i].groupId
      //      })
      //    }
      //   await Post.update({ 
      //     _id:result.groups[i].groupId
